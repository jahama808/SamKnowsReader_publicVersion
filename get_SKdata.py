...
data_download = '{"splits":["unit"],"metric":"httpgetmt","chartType":"aggregate","aggregation":"hourly","normalised":true,"prefilter":true,"filters":[{"id":"isp","filterType":1,"filterValues":["372"]}],"time":{"from":"-2 days","to":"now"},"agent_agg":false}'
data_upload = '{"splits":["unit"],"metric":"httppostmt","chartType":"aggregate","aggregation":"hourly","normalised":true,"prefilter":true,"filters":[{"id":"isp","filterType":1,"filterValues":["372"]}],"time":{"from":"-2 days","to":"now"},"agent_agg":false}'
data_udplatency = '{"splits":["unit"],"metric":"udpLatency","chartType":"aggregate","aggregation":"hourly","normalised":true,"prefilter":true,"filters":[{"id":"isp","filterType":1,"filterValues":["372"]}],"time":{"from":"-2 days","to":"now"},"agent_agg":false}'
...
for i in range(0,size_of_access_data):
    ...
    if number_below_min > 1:
        fail_rate = int(float(number_below_min)/float(total_entries)*100)
        print("whitebox: {} has {} below threshold of {} Kbps out of {} entries.  Fail Rate: {}".format(target,number_below_min,threshold,total_entries,fail_rate))
        access_data.loc[(access_data['unitID']==int(target)),'failrateDownload']= fail_rate
        print(low_down_df)
    else:
        if total_entries > 0:
            fail_rate = int(float(number_below_min)/float(total_entries)*100)
    ...
    if number_below_min_upload > 1:
        fail_rate = int(float(number_below_min_upload)/float(total_entries)*100)
        print("whitebox: {} has {} below threshold of {} Kbps out of {} entries.  Fail Rate: {}".format(target,number_below_min_upload,upload_threshold,total_entries,fail_rate))
        access_data.loc[(access_data['unitID']==int(target)),'failrateUpload']= fail_rate
        print(low_up_df)
    else:
        if total_entries > 0:
            fail_rate = int(float(number_below_min_upload)/float(total_entries)*100)
    ...
    if number_above_max_latency > 1:
        fail_rate = int(float(number_above_max_latency)/float(total_entries)*100)
        print("whitebox: {} has {} below threshold of {} ms out of {} entries.  Fail Rate: {}".format(target,number_above_max_latency,latency_threshold,total_entries,fail_rate))
        access_data.loc[(access_data['unitID']==int(target)),'failrateLatency']= fail_rate
        print(high_delay_df)
    else:
        if total_entries > 0:
            fail_rate = int(float(number_above_max_latency)/float(total_entries)*100)
